...
### 1.1 设计与抽象
- [x] 定义统一攻击结算流水线：命中检定 -> 伤害构建 -> 减免吸收 -> 最终结算 -> 事件输出
- [x] 设计 `CombatSnapshot`（战斗快照）结构：基础属性、装备加成、状态加成、地形修正、临时效果
- [x] 设计 `DamagePacket` 结构：来源、目标、伤害类型、基值、倍率、穿透、可否暴击
- [x] 设计 `MitigationResult` 结构：被哪些机制减免/吸收、每一步数值变化
- [x] 设计统一结果对象：用于前端展示、日志、调试面板和回放

### 1.2 落地改造
- [x] 将怪物回合攻击接入统一求值层
- [x] 将玩家攻击在后端形成权威结算接口
- [x] 补充“命中失败”“暴击”“免疫”“护盾吸收”等标准事件文本
- [x] 新旧结算并行一段时间，输出对比日志（用于校验）

### 1.3 测试
- [x] 构建黄金样例（固定种子 + 固定场景）验证结算稳定性
- [x] 对比旧系统误差并评估体感（过强/过弱）
- [ ] 压测回合推进性能，确认不会显著增加响应延迟

**验收标准**
- [x] 后端任意攻击都可产出完整 breakdown
- [x] 同输入同种子结算可复现

#### 1.x 本轮风险修复（补充）

- [x] `combat_authority_mode=local` 下后端 attack 改为仅返回预测，不落账，避免双端重复结算
- [x] 攻击结算支持 `deterministic_seed`，稳定同输入复现和 hybrid 对比基线
- [x] 抗性/易伤/免疫字段纳入角色模型与存档读写链，跨存档保持一致
