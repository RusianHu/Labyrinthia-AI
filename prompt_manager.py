...
- "snowfield": 雪地（白雪+冰晶）- 适合雪原、冰川、寒冷地带、冬季场景
- "town": 城镇街道（石板路+裂纹）- 适合城镇、村庄、街道、人类聚居地

**选择指南**：
1. 如果任务描述包含"魔法"、"法师"、"神殿"、"符文"等关键词 → 使用 "magic"
2. 如果任务描述包含"废弃"、"遗忘"、"古老"、"破败"等关键词 → 使用 "abandoned"
3. 如果任务描述包含"洞穴"、"地下"、"潮湿"、"天然"等关键词 → 使用 "cave"
4. 如果任务描述包含"战斗"、"竞技场"、"血腥"、"屠杀"等关键词 → 使用 "combat"
5. 如果任务描述包含"草原"、"森林"、"野外"、"自然"等关键词 → 使用 "grassland"
6. 如果任务描述包含"沙漠"、"荒地"、"干旱"等关键词 → 使用 "desert"
7. 如果任务描述包含"农场"、"田野"、"村庄"、"种植"等关键词 → 使用 "farmland"
8. 如果任务描述包含"雪"、"冰"、"寒冷"、"冬季"等关键词 → 使用 "snowfield"
9. 如果任务描述包含"城镇"、"街道"、"村庄"、"聚居地"等关键词 → 使用 "town"
10. 如果以上都不匹配，可根据你的理解进行适当的选择，也可以使用 "normal" 作为默认选项

**重要**：请仔细阅读任务信息中的"建议主题"、"任务描述"和"故事背景"，根据这些信息选择最贴切的floor_theme！
            """.strip(),
